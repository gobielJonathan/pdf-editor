...
// Resolve worker path relative to the current page so it works under both
// http:// (browser) and file:// (Electron packaged app).
  ...
  async function applyEdits() {
    ...
    for (const [pageNum, slot] of pages.entries()) {
      ...
      for (const [editId, newText] of slot.edits.entries()) {
        ...
        const origMaxY =
          Math.max(...para.items.map((i) => i.pdfY + i.pdfFontSize * 1.2)) + baseFs * 0.2;
        libPage.drawRectangle({
          x: origMinX,
          y: origMinY,
        ...
        const drawX =
          fmt.left != null ? fmt.left / scale : Math.min(...para.items.map((i) => i.pdfX));
        const drawY =
          fmt.top != null
      ...
      for (const addition of slot.additions ?? []) {
        ...
        if (addition.type === "text" && addition.text?.trim()) {
          ...
          (addition.type === "image" || addition.type === "signature") &&
          addition.dataUrl
        ) {
          const pdfX = addition.left / scale;
